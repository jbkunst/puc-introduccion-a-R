...
- Obtenga la distribución de mujeres y hombres en las ventas, es decir cuantos créditos de consumo
fueron vendidos a mujeres y cuantos a hombres.
...
- Estudie si existe diferencia del ingreso según género y comente.
...
```{r, include=FALSE}
clientes_edad <- read_csv("data/clientes-edad.csv")
```
...
que los datos consolidados (con edad) se llaman `clientes` use lo siguiente:
...
arbol <- ctree(malo_en_los_12m ~ ., data = clientes, control = ctree_control(maxdepth = 4))
```

- Comente acerca de que variables fueron las más importantes/relevantes.

- Comente que perfiles de clientes son más propensos a ser _malos_ en los 12 meses siguientes a
la obtención de un crédito de consumo.
...
```{r, include=FALSE}
clientes_nuevos <- read_csv("data/clientes-nuevos.csv")
```
...
Ahora usando él siguiente código, que entrega la probabilidad de que sea o no malo en los próximos siguientes
12 meses, indique cual es el cliente más propenso a serlo y por comenté la razón de eso: 

```
predict(arbol, clientes_nuevos, type = "prob")
```
